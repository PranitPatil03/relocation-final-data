...
const tempFile = "./temp/results_temp_1.json";
...
(async () => {
  try {
    ...
    const sourceFile = "./finaldata/s/split_city_code_master_1.json"; // Replace with your actual source file
    const destinationFile = "./finaldata/d/split_city_code_master_1.json"; // Replace with your actual destination file

    const sourceData = JSON.parse(fs.readFileSync(sourceFile, "utf-8"));
    const destinationData = JSON.parse(
      fs.readFileSync(destinationFile, "utf-8")
    );
    ...
    if (fs.existsSync(tempFile)) {
      ...
      console.log(
        `Resuming from previously saved progress. ${results.length} pairs processed.`
      );
    }
    ...
    for (let i = results.length; i < sourceData.length; i++) {
      console.log(
        `Processing pair ${i + 1} of ${totalPairs}: ${sourceData[i].city} to ${
          destinationData[i].city
        }`
      );
      ...
      if (result) {
        results.push(result);
        console.log(
          `Data fetched successfully for: ${sourceData[i].city} to ${destinationData[i].city}`
        );
        ...
        console.warn(
          `Skipping pair due to error: ${sourceData[i].city} to ${destinationData[i].city}`
        );
      }
    }

    // Save final results and remove temp file
    const outputFile = "./finalR/results_1.json";
